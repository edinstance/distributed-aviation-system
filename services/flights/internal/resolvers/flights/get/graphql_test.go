...
func (m *MockFlightService) GetFlightByID(
	ctx context.Context,
	id uuid.UUID,
) (*models.Flight, error) {
...
func TestFlightResolverGetFlight(t *testing.T) {
	id := uuid.New()
	number := "AA123"
	origin := "LAX"
	destination := "JFK"
	...
	tests := []struct {
		...
		},
		{
			...
			id:   id.String(),
			serviceSetup: func(m *MockFlightService) {
				m.On("GetFlightByID", mock.Anything, mock.Anything).Return(nil, errors.New("db error"))
			},
			expectErr:     true,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			flight, err := resolver.GetFlightById(
				context.Background(),
				tc.id,
			)
