...
var require_validate2 = __commonJS({
  "node_modules/graphql/validation/validate.js"(exports) {
    ...
    function validate(schema, documentAST, rules = _specifiedRules.specifiedRules, options2, typeInfo = new _TypeInfo.TypeInfo(schema)) {
  ...
  "node_modules/graphql/execution/execute.js"(exports) {
    ...
    function buildExecutionContext(args) {
      ...
      const coercedVariableValues = (0, _values.getVariableValues)(
        ...
        {
          maxErrors: (_options$maxCoercionE = options2 === null || options2 === void 0 ? void 0 : options2.maxCoercionErrors) !== null && _options$maxCoercionE !== void 0 ? _options$maxCoercionE : 50
        }
      );
...
// src/helpers/graphql_request.ts
var import_graphql = __toESM(require_graphql2(), 1);
import http from "k6/http";
import { check } from "k6";
function graphql(url, doc, variables) {
  const res = http.post(
    url,
    JSON.stringify({
      query: (0, import_graphql.print)(doc),
      variables
    }),
...
function randomFlightNumber() {
  ...
  return code + Math.floor(100 + Math.random() * 9e3);
}
function runFlightScenario(url, aircraftId) {
  graphql(
    url,
    GetFlightByIdDocument,
    { id: uuidv4() }
  );
  const createRes = graphql(url, CreateFlightDocument, {
    aircraftId,
    number: randomFlightNumber(),
    origin: "LHR",
    destination: "JFK",
    departureTime: (/* @__PURE__ */ new Date()).toISOString(),
    arrivalTime: new Date(Date.now() + 3600 * 1e3).toISOString()
  });
  check2(createRes, {
...
// src/tests/shells/aircraft.ts
import { uuidv4 as uuidv42 } from "https://jslib.k6.io/k6-utils/1.4.0/index.js";
import { check as check3 } from "k6";
function runAircraftScenario(url) {
  graphql(
    url,
    GetAircraftDocument,
    { id: uuidv42() }
  );
  const createAircraftRes = graphql(url, CreateAircraftDocument, {
    input: {
      registration: `N-${uuidv42().slice(0, 8)}`,
      manufacturer: "Boeing",
      model: "737",
      capacity: 50,
